...
# save fig
intertopic_distance.write_html("bert_corp_news_intertopic.html")
```

```{r}
htmltools::includeHTML("bert_corp_news_intertopic.html")
```
...
```{r}
htmltools::includeHTML("fig_overtime.html")
```
...
pred <- zero_shot_classification(
  ...
  c("negative sentiment", "positive sentiment")
)
...
results <- pred |> 
  ...
  mutate(truth = corp_movies_test$sentiment[1:10])
